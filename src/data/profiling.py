...
def karaman_davis(pkidb_ligands=False, fda_approved=False, activity_max=100):
    ...
    print(f"Karaman matrix: {df_karaman.shape}")
    df_davis = davis(pkidb_ligands, fda_approved)
    print(f"Davis matrix: {df_davis.shape}")

    print(f"Number of shared ligands: {len(set(df_karaman.columns) & set(df_davis.columns))}")
    print(f"Number of shared kinases: {len(set(df_karaman.index) & set(df_davis.index))}")

    print(f"Number of total ligands: {len(set(df_karaman.columns).union(set(df_davis.columns)))}")
    print(f"Number of total kinases: {len(set(df_karaman.index).union(set(df_davis.index)))}")
    ...
    print(f"Karaman-Davis matrix: {df_combined.shape}")
